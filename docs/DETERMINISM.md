...
- `u32Add` and `u32Mul` return the low unsigned 32 bits. Inputs must be uint32 values; malformed inputs throw.
...
- `splitU64` and `joinU64` are the canonical high/low uint32 bridge for zero through `2^64 - 1`, including totals beyond host-safe integers.
...
- `tickToMinuteOfDay` uses nonnegative integer ticks and an explicit positive epoch length. `DeterministicClock` advances only by explicit nonnegative deltas; neither API reads a wall clock.
